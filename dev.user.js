...
(function () {
  const url = `http://localhost:%PORT%/out.js?${Date.now()}`
  new Promise(function loadBundleFromServer (resolve, reject) {
    ...
    if (e && 'status' in e) {
      if (e.status <= 0) {
        log('Server is not responding at ' + url)
        GM.getValue('scriptlastsource3948219', false).then(function (src) {
          if (src) {
    ...
    if (s) {
      ...
      eval(`${s}
//# sourceURL=${url}`)
      GM.setValue('scriptlastsource3948219', s)
    }
  })
})()
